#!/bin/bash
# Re-enqueue user jobs for repositories in users_processing state with GCP secrets
# Usage: ./reenqueue-user-jobs-gcp.sh
...
if [ -z "$PROJECT_ID" ] || [ "$PROJECT_ID" = "YOUR_GCP_PROJECT_ID" ]; then
  ...
  echo "      PROJECT_ID=\"your-actual-project-id\" ./reenqueue-user-jobs-gcp.sh"
  echo ""
  exit 1
fi
