import { commitQueue, waitForQueueReady } from '../services/queueService';
 ...
 * Processes multiple commit jobs from the queue per execution (configurable)
 * Processes repos in parallel batches for better throughput while staying within free tier limits
 */
...
const MAX_CONCURRENT_JOBS = parseInt(
  ...
  10
); // Process up to 10 repos concurrently (configurable)

async function processCommitJobs() {
  try {
    // Wait for Redis connection to be ready before proceeding
    logger.info('[COMMIT_WORKER] Waiting for Redis connection to be ready...');
    await waitForQueueReady(commitQueue, 10, 2000);
    logger.info('[COMMIT_WORKER] Redis connection ready');

    // First, check for stuck active jobs (jobs that were interrupted)
    let activeJobs: any[] = [];
    try {
      activeJobs = await commitQueue.getActive();
    } catch (error: any) {
      logger.error(
        `[COMMIT_WORKER] Failed to get active jobs: ${error.message}`
      );
    ...
    let waitingJobs: any[] = [];
    try {
      waitingJobs = await commitQueue.getWaiting();
    } catch (error: any) {
      logger.error(
        `[COMMIT_WORKER] Failed to get waiting jobs: ${error.message}`
      );
    ...
    logger.info(
      `[COMMIT_WORKER] Processing ${jobsToProcess.length} job(s) in parallel batches (max per execution: ${MAX_JOBS_PER_EXECUTION}, concurrent: ${MAX_CONCURRENT_JOBS}, total waiting: ${waitingJobs.length})`
    );
    ...
    const processJob = async (job: any, index: number): Promise<void> => {
      logger.info(
        `[COMMIT_WORKER] [${index + 1}/${jobsToProcess.length}] Starting job ${job.id} for repository: ${job.data.url}`
      );
      ...
      try {
        ...
        while (checkCount < maxChecks) {
          const currentJob = await commitQueue.getJob(job.id);
          if (!currentJob) {
          ...
          if (
            ...
            logger.info(`[COMMIT_WORKER] Job ${job.id} is now ${jobState}`);
            break;
          }

          if (checkCount % 5 === 0) {
            logger.info(
              `[COMMIT_WORKER] Waiting for job ${job.id} to be picked up by processor (state: ${jobState}, attempt ${checkCount + 1}/${maxChecks})...`
            );
          }
        ...
        if (jobState === 'waiting' || jobState === 'delayed') {
          logger.error(
            `[COMMIT_WORKER] Job ${job.id} was never picked up by processor (final state: ${jobState})`
          );
          throw new Error(
        ...
        processedCount++;
        logger.info(
          `[COMMIT_WORKER] ✅ Successfully processed job ${job.id} (${processedCount}/${jobsToProcess.length} completed)`
        );
      } catch (error: any) {
        failedCount++;
        logger.error(
          `[COMMIT_WORKER] ❌ Failed to process job ${job.id}: ${error.message}`
        );
    ...
    for (let i = 0; i < jobsToProcess.length; i += MAX_CONCURRENT_JOBS) {
      ...
      logger.info(
        `[COMMIT_WORKER] Processing batch ${Math.floor(i / MAX_CONCURRENT_JOBS) + 1} (${batch.length} jobs concurrently)`
      );
      await processBatch(batch, i);
    }

    logger.info(
      `[COMMIT_WORKER] ✅ Batch complete! Processed: ${processedCount}, Failed: ${failedCount}, Remaining in queue: ${Math.max(0, waitingJobs.length - jobsToProcess.length)}`
    );
...
async function main() {
  try {
    ...
    logger.info(
      `[COMMIT_WORKER] Starting job processing (max ${MAX_JOBS_PER_EXECUTION} jobs per execution, ${MAX_CONCURRENT_JOBS} concurrent)...`
    );
    await processCommitJobs();
  } catch (error: any) {
